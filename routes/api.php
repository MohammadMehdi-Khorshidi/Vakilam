...
Route::prefix('auth/register')->name('apiRegister.')->controller(RegisterController::class)
    ->group(function () {
...
Route::prefix('auth')->name('apiAuth.')->controller(LoginController::class)
    ->group(function () {
        Route::post('/login', 'store')->middleware('throttle:5,1')->name('login');
        Route::post('/logout', 'destroy')->middleware('auth:sanctum')->name('logout');
    });

Route::prefix('auth/password')->name('apiPassword.')->controller(PasswordResetController::class)
    ->group(function () {
...
Route::middleware(['auth:sanctum', 'active'])->controller(ClientDashboardController::class)
    ->group(function () {
        Route::get('/client/dashboard', 'index');
    });

Route::middleware(['auth:sanctum', 'active'])->controller(ClientCaseController::class)
    ->group(function () {
        Route::get('/client/cases/{legalMatter}', 'show');
    });

Route::controller(LocationReferenceController::class)->prefix('reference')
    ->group(function () {
...
Route::middleware(['auth:sanctum', 'active'])->controller(LegalRequestServiceIntentController::class)
    ->group(function () {
        Route::get('/legal-requests/{legalRequest}/service-options', 'options');
        Route::post('/legal-requests/{legalRequest}/service-intent', 'store');
    });

Route::middleware(['auth:sanctum', 'active'])->controller(LawyerMatchingController::class)
    ->group(function () {
