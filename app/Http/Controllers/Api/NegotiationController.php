...
class NegotiationController extends Controller
{
    ...
    public function close(Request $request, Negotiation $negotiation): JsonResponse
    {
        ...
        $negotiation = DB::transaction(function () use ($negotiation): Negotiation {
            ...
            abort_unless(
                in_array($locked->status, [Negotiation::STATUS_ACTIVE, Negotiation::STATUS_PROPOSAL_SUBMITTED], true),
                409,
            ...
            if ($locked->proposal !== null && in_array($locked->proposal->status, [
                LawyerProposal::STATUS_DRAFT,
                LawyerProposal::STATUS_SUBMITTED,
                LawyerProposal::STATUS_SHORTLISTED,
            ], true)) {
                $locked->proposal->forceFill(['status' => LawyerProposal::STATUS_CANCELLED])->save();
            }
        ...
        $proposal = DB::transaction(function () use ($request, $negotiation): LawyerProposal {
            ...
            if ($existing !== null) {
                abort_unless(
                    ...
                    409,
                    'This lawyer already has a proposal for the legal request.',
                );
                abort_unless(
