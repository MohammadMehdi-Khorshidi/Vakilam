...
export default function StepUrgency({ data, update }) {
    return (
        <div dir="rtl" className={vazir.className}>
            <div className="mb-5 rounded-xl bg-[#e8f2ef] p-4 font-bold">میزان فوریت مسئله را مشخص کنید.</div>
            <div className="grid gap-3 md:grid-cols-2">
                {urgencyOptions.map((item) => (
                    <ChoiceButton key={item.value} active={data.urgency === item.value} onClick={() => update('urgency', item.value)}>
                        {item.label}
                    </ChoiceButton>
                ))}
            </div>
