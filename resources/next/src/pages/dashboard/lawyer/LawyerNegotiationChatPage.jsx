...
export default function LawyerNegotiationChatPage({ negotiationId }) {
  ...
  const {otherOnline,otherTyping,syncError,notifyTyping,refreshNow} = useNegotiationRealtime({
    negotiationId,
        ...
        <header className="rounded-[20px] border border-[#dce6e2] bg-white p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {negotiation.engagement ? <button type="button" onClick={()=>{ if (negotiation?.engagement?.public_id) window.location.href=`/lawyer/engagement/${negotiation.engagement.public_id}`; }} className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-bold text-emerald-700 transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-400 hover:bg-emerald-100 hover:shadow-sm active:translate-y-0"><Handshake size={15}/>مدیریت توافق و قرارداد</button> : canCreateProposal ? <button type="button" onClick={()=>setProposalOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-[#c7a154] px-4 py-2.5 text-sm font-bold text-[#173f38]"><FileSignature size={17}/>ثبت پیشنهاد رسمی</button> : null}
            </div>
          </div>
  ...
  <input type="text" inputMode="numeric" value={formatMoneyInput(proposalForm.proposed_fee_rial)}
    onChange={(e)=>setProposalForm((p)=>({...p,proposed_fee_rial:normalizeMoneyInput(e.target.value)}))}
    className="h-[48px] w-full rounded-xl border p-3" placeholder="مبلغ به ریال"/>
  <p className="mt-1.5 text-xs text-slate-500">مثال: 10,000,000 ریال</p>
</div><input type="number" min="1" value={proposalForm.estimated_days} onChange={e=>setProposalForm(p=>({...p,estimated_days:e.target.value}))} className="h-[48px] w-full rounded-xl border p-3" placeholder="زمان تقریبی (روز)"/></div><button type="button" onClick={submitProposal} disabled={proposalBusy} className="w-full rounded-xl bg-[#174c42] px-5 py-3.5 font-bold text-white disabled:opacity-50">{proposalBusy?'در حال ثبت...':'ثبت و ارسال پیشنهاد رسمی'}</button></div></div></div> : null}
    </main>
  );
}
