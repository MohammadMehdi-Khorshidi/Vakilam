...
import { getClientLawyerRequests, getLegalRequest, getLegalRequestProposals } from '@/lib/api/legalRequests';

const vazirmatn = Vazirmatn({ subsets: ['arabic'], display: 'swap' });
const faNumber = new Intl.NumberFormat('fa-IR');
const ACTIVE_INVITATION_STATUSES = new Set(['pending', 'negotiating']);
const statusLabels = { draft:'پیش‌نویس', submitted:'در انتظار پاسخ وکلا', matched:'در حال بررسی وکلا', active:'در جریان', in_progress:'در حال پیگیری', closed:'بسته‌شده', cancelled:'لغوشده' };
const invitationStatusLabels = { pending:'در انتظار پاسخ', negotiating:'مذاکره فعال', rejected:'رد شده توسط وکیل', expired:'منقضی شده', closed:'بسته شده', selected:'انتخاب نهایی' };
const urgencyLabels = { low:'کم', normal:'عادی', high:'زیاد', urgent:'فوری' };
const serviceIntentLabels = { consultation:'مشاوره حقوقی', lawyer_selection:'انتخاب وکیل' };

function formatDate(value) { if (!value) return 'ثبت نشده'; try { return new Intl.DateTimeFormat('fa-IR',{year:'numeric',month:'long',day:'numeric'}).format(new Date(value)); } catch { return 'ثبت نشده'; } }
